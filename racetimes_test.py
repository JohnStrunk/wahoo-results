...
# Copyright (C) 2022-2024 - John D. Strunk
#
...
from racetimes import DO4, NumericTime, RaceTimes, Time
...
def test_can_parse_header(do4_mising_one_time) -> None:
    ...
    race: RaceTimes = DO4(do4_mising_one_time, 2, NumericTime(0.30), now, meet_seven)
...
def test_resolve_times(do4_mising_one_time) -> None:
    ...
    assert race.final_time(1).value == NumericTime("143.37")

    assert len(race.times(3)) == 3
    assert race.final_time(3).is_valid
    assert race.final_time(3).value == NumericTime("128.14")


def test_toofew_times(do4_mising_one_time) -> None:
    """Final time is invalid if too few raw times"""
    race: RaceTimes = DO4(do4_mising_one_time, 3, NumericTime("0.30"), now, meet_seven)
    assert len(race.times(3)) == 3
...
def test_largedelta_times(do4_big_delta) -> None:
    ...
    race: RaceTimes = DO4(do4_big_delta, 2, NumericTime("0.30"), now, meet_seven)
    assert len(race.times(1)) == 3
    assert not race.final_time(1).is_valid
    assert race.final_time(1).value == NumericTime("130.63")

    times = race.times(1)
    assert times[0] == Time(NumericTime("160.72"), False)
    assert times[1] == Time(NumericTime("130.63"), True)
    assert times[2] == Time(NumericTime("130.61"), True)


def test_one_zero_times(do4_one_time) -> None:
    """Ensure we can calculate final times correctly"""
    race: RaceTimes = DO4(do4_one_time, 2, NumericTime("0.30"), now, meet_seven)

    assert race.times(1) == [None, Time(NumericTime("55.92"), True), None]
    assert not race.final_time(1).is_valid
    assert race.final_time(1).value == NumericTime("55.92")

    assert len(race.times(7)) == 3
    assert not race.final_time(7).is_valid
    assert race.final_time(7).value == NumericTime("0")
...
def test_places() -> None:
    ...
    race: RaceTimes = DO4(data, 2, NumericTime("0.30"), now, meet_seven)
